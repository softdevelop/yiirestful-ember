...
class Serializer extends Component
{
    ...
    protected function serializeModel($model)
    {
        if ($this->request->getIsHead()) {
            ...
            $longModelName = get_class($model);  
            $arrModelName = explode("\\", $longModelName);
            $modelName = $arrModelName[count($arrModelName)-1];
            $data = array();
            $data[$modelName] = $model->toArray($fields, $expand);
            return $data;
